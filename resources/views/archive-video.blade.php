...
$imageAspectRatio = get_field('image_aspect_ratio', 'option') ?? 'aspect-video';
$showExcerpt = get_field('show_excerpt', 'option') ?? true;
$showCategory = get_field('show_category', 'option') ?? false;
$makeCardClickable = get_field('make_card_clickable', 'option') ?? false;
@endphp
    ...
    <div class="videos-grid">
      ...
      <div class="u-max-width-70ch mx-auto text-center">
        <p class="u-text-style-medium mb-u-6">
          {!! __('Sorry, no videos were found.', 'sage') !!}
        </p>
...
{{-- Flexible Content Blocks from Videos Archive Page --}}
@php
$videosPageId = get_field('page_for_videos', 'option');

if ($videosPageId && have_rows('content_blocks', $videosPageId)) {
    while (have_rows('content_blocks', $videosPageId)) {
        the_row();
