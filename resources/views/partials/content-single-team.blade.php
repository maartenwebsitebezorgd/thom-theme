@php
// Get ACF fields from team member
...
$imageAspectRatio = get_field('team_single_image_aspect_ratio', 'option') ?: 'aspect-auto';

$name = get_the_title();
$fullName = trim(($firstName ?? '') . ' ' . ($lastName ?? '')) ?: get_the_title();
$bio = get_the_content();
$excerpt = get_the_excerpt();
@endphp

<article {!! post_class('h-entry') !!} itemscope itemtype="https://schema.org/Person">

  <header data-theme="{{ $headerTheme }}" class="u-section pt-section-main pb-section-main">
    <div class="u-container max-w-container-main">
      <div class="team-member-header grid grid-cols-1 lg:grid-cols-2 gap-u-8 items-start">

        {{-- Photo Column --}}
        @if($headshot)
        <div class="team-photo-wrapper">
          <div class="{{ $imageAspectRatio }} overflow-hidden">
            <img
              src="{{ $headshot['url'] }}"
              alt="{{ $headshot['alt'] ?: $fullName }}"
        ...
        {{-- Info Column --}}
        <div class="team-info u-margin-trim">
          <h1 class="p-name u-text-style-h2 mb-u-4" itemprop="name">{{ $name }}</h1>

          @if($jobTitle)
          <p class="u-text-style-h6 text-[var(--theme-accent)] mb-u-6" itemprop="jobTitle">{{ $jobTitle }}</p>
          @endif

          @if($bio)
          <div class="u-text-style-main mb-u-6 prose" itemprop="description">
            {!! apply_filters('the_content', $bio) !!}
          </div>
          @endif

          {{-- Contact Info --}}
          <div class="contact-info flex flex-col gap-u-3 mb-u-6">
            @if($email)
            <a href="mailto:{{ $email }}" class="inline-flex items-center gap-u-2 hover:text-[var(--theme-accent)] transition-colors" itemprop="email">
            ...
            @if($phone)
            <a href="tel:{{ $phone }}" class="inline-flex items-center gap-u-2 hover:text-[var(--theme-accent)] transition-colors" itemprop="telephone">
              <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div class="social-links flex gap-u-3">
            ...
            <a
              ...
              class="inline-flex items-center justify-center w-12 h-12 rounded-full bg-[var(--theme-text)]/10 hover:bg-[var(--theme-accent)] hover:text-white transition-colors"
