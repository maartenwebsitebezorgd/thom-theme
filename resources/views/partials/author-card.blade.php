...
*
* @param array $teamMemberAuthors - Array of team member post IDs (supports single or multiple)
* @param int $teamMemberAuthorId - (Deprecated) Single ID for backwards compatibility
...
$authorIds = [];
if (isset($teamMemberAuthors) && is_array($teamMemberAuthors)) {
$authorIds = $teamMemberAuthors;
} elseif (isset($teamMemberAuthorId)) {
$authorIds = [$teamMemberAuthorId];
}
...
<section data-theme="{{ $contentTheme }}" class="u-section pt-section-tiny pb-section-main">
  <div class="u-container max-w-container-main">
    <div class="{{ $contentMaxWidth }} mx-auto pt-u-5 border-t border-neutral-300">
      ...
      <div class="authors-grid grid grid-cols-1 {{ $authorCount > 1 ? 'sm:grid-cols-2' : '' }} gap-u-6">
        @foreach($authorIds as $authorId)
        @php
        // Get author data
        $authorName = get_the_title($authorId);
        $authorJobTitle = get_field('job_title', $authorId);
        $authorHeadshot = get_field('headshot', $authorId);
        $authorBio = get_the_excerpt($authorId);
        $authorEmail = get_field('email', $authorId);
        $authorPhone = get_field('phone', $authorId);
        $authorSocialLinks = get_field('social_links', $authorId);
        $authorPermalink = get_permalink($authorId);
        @endphp

        <div class="author-card flex flex-row gap-u-5 items-start">
          @if($authorHeadshot)
